...
// and yet not necesary at all.
// this happens for mis understanding the requirements.
export async function courseManipulation({ form }, data, courseData) {
  ...
  if (form.docFiles !== undefined && form.docFiles !== null) {
    for (let i = 0; i < form.docFiles.length; i++) {
      const filePath = `${courseDocFileFolder}/${Date.now()}_${
        form.docFiles[i].name
      }`;
    ...
    docFileArray = form.docFiles;
  } else {
    docFileArray = data.docFiles;
  }

  let videoFileArray = [];
  if (form.videoFiles !== undefined && form.videoFiles !== null) {
    for (let i = 0; i < form.videoFiles.length; i++) {
      const filePath = `${courseVideoFileFolder}/${Date.now()}_${
        form.videoFiles[i].name
      }`;
    ...
    videoFileArray = form.videoFiles;
  } else {
    videoFileArray = data.videoFiles;
  }
